...
export function ThemeToggle() {
  ...
  if (!mounted) {
    return (
      <div className="w-8 h-8 rounded-full bg-neutral-200/50 dark:bg-neutral-800/50" />
    );
  }
  ...
  return (
    ...
    >
      {isDark ? (
        <Sun className="w-4 h-4 transition-transform duration-200 text-amber-400" />
      ) : (
        <Moon className="w-4 h-4 transition-transform duration-200 text-slate-700 dark:text-slate-200" />
      )}
    </button>
  );
}
